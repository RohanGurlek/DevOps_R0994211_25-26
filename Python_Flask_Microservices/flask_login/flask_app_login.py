...
def signup_v2():
    ...
    try:
        ...
        password_entered = (request.form.get('password').encode()).hexdigest()
        hash_value = hashlib.sha256(password_entered)
        sql_statement = "INSERT INTO USER_HASH VALUES ('{0}' , '{1}' ) ".format(username_entered, hash_value)
        c.execute(sql_statement)
        db_conn.commit()
    except sqlite3.IntegrityError:
